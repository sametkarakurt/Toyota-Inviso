...
import {NativeBaseProvider, Center, ScrollView, Button,Box,FlatList} from 'native-base';
...
const FormScreen = ({route,navigation}) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
     ...
     setTypeArray(oldArray => [...oldArray, {itemType:'submitButton',itemOptions:null,func:null}])

      
    
      data[0].pages[0].forms.forEach(items => {
        items.rows.forEach((item) => {
          item.components.forEach(comp=> {
            if(comp.options[0]){
              
              console.log(comp.options[0].key)
              ...
              fields.forEach(item => {
                ...
                if(item.value ===  comp.options[0].key ){
                  ...
                  setTypeArray(oldArray => [...oldArray, {itemType:item.type,itemOptions:comp.options,func:item.func}])
                }
              ...
              fields.forEach(item => {
                ...
                if(item.value ===  comp.key){
                  ...
                  setTypeArray(oldArray => [...oldArray, {itemType:item.type,itemOptions:null,func:item.func}])
                }
